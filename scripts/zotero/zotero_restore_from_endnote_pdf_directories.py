...
# -*- coding: utf-8 -*-
"""
Zotero restore agent from EndNote PDF directories (PDF/XXX/*)
...
OCR:
- If PDF extracted text too short, try OCR (ocrmypdf) then re-extract.
RIS:
- Authors are in ONE LINE: "AU  - A, B, C"
- Attachments: write multiple L1 lines (primary first) + N1 note listing all attachments.

Use .env fallback if CLI args not provided.

Example:
  python zotero_restore_from_endnote_pdf_directories.py --enable_ocr --ocr_lang eng
  python zotero_restore_from_endnote_pdf_directories.py --enable_ocr --ocr_lang eng+chi_sim
  python zotero_restore_from_endnote_pdf_directories.py --env_file .env.ollama --enable_ocr --ocr_lang eng+chi_sim
  
  python zotero_restore_from_endnote_pdf_directories.py `
  --provider ollama_native `
  --base_url http://localhost:11434 `
  --model "yasserrmd/Qwen2.5-7B-Instruct-1M" `
  --root_dir "C:/Users/shuif/Desktop/PDF" `
  --out_dir "./outputs_ollama" `
  --enable_ocr --ocr_lang eng+chi_sim
...
from zr.ris_export import write_record_outputs
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Restore Zotero records from EndNote PDF/XXX/* directories.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    p.add_argument("--dotenv", default=".env", help="Path to .env (optional).")
    p.add_argument("--root_dir", default=None,
                   help="EndNote PDF root directory containing subfolders (PDF/XXX/*). "
                        "If omitted, fallback to PDF_DIR in .env/environment.")
...
def process_single_record(folder, provider, cfg, state, out_dir, log, progress_lock, counters):
    ...
    try:
        ...
        # Write outputs (RIS + intermediate JSON)
...
def main():
    ...
    # Load .env (relative to script directory if needed)
    dotenv_path = pathlib.Path(args.dotenv).expanduser()
    if not dotenv_path.is_absolute():
        dotenv_path = pathlib.Path(__file__).resolve().parent / dotenv_path
